...
interface ERC20{
    ...
    function transferForm(address sender, address recipient,uint 256 amount) external returns(bool);
...
contract TokenICO{
    ...
    function updateTokenSalePrice(uint256 _tokenSalePrice){
    ...
    function buyToken(uint256 _tokenAmount ) public payable{
        require(msg.value == multiply(_tokenAmount,tokenSalePrice),"Insufficient ethers provided for the purchase for token");
    ...
    function transferToOwner(uint _amount) external payable{
        require(msg.value >= amount,"Insufficent fund send to owner");
        (bool success,) = owner.call{value: _amount}("");
        require(success,"Trasaction faild");
